...
//轮密钥
uint32_t rK[32], rk[32];

//明文
uint8_t plaintext[16] = { 0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,
                           0xfe,0xdc,0xba,0x98,0x76,0x54,0x32,0x10 };
uint32_t plaintext1[4] = { 0x01234567,0x89abcdef,0xfedcba98,0x76543210 };
static void dump_buf1(uint8_t* buf, uint32_t len)
{
    int i;
    for (i = 0; i < len; i++) {
...
uint8_t S_replace(uint8_t in)
{
    ...
    uint8_t ycoor = in << 4;
    ycoor = ycoor >> 4;
    int index = xcoor * 16 + ycoor;
    return SBOX[index];
}

//左移操作
uint32_t Cycle_Shift_lift(uint32_t num, int Shift_number) {
    return (num << Shift_number) ^ (num >> 32 - Shift_number);
}

//密钥生成时的T函数
uint32_t T_for_get_key_func(uint32_t K_for_t) {
    uint8_t* A = (uint8_t*)&K_for_t;
    uint8_t B_8_t[4];
    B_8_t[0] = S_replace(A[0]);
    B_8_t[1] = S_replace(A[1]);
    B_8_t[2] = S_replace(A[2]);
    B_8_t[3] = S_replace(A[3]);
    uint32_t* B_32_t = (uint32_t*)B_8_t;
...
//轮函数中的T函数
uint32_t T_for_roundfunc_func(uint32_t K_for_t) {
    uint8_t* A = (uint8_t*)&K_for_t;
    uint8_t B_8_t[4];
    B_8_t[0] = S_replace(A[0]);
    B_8_t[1] = S_replace(A[1]);
    ...
    B_8_t[3] = S_replace(A[3]);
    uint32_t* B_32_t = (uint32_t*)B_8_t;
    uint32_t B_32_t_Shift_lift_2 = Cycle_Shift_lift(*B_32_t, 2);
    uint32_t B_32_t_Shift_lift_10 = Cycle_Shift_lift(*B_32_t, 10);
    uint32_t B_32_t_Shift_lift_18 = Cycle_Shift_lift(*B_32_t, 18);
    uint32_t B_32_t_Shift_lift_24 = Cycle_Shift_lift(*B_32_t, 24);
    return *B_32_t ^ B_32_t_Shift_lift_2 ^ B_32_t_Shift_lift_10 ^ B_32_t_Shift_lift_18 ^ B_32_t_Shift_lift_24;
}
static uint32_t table0[256];
static uint32_t table1[256];
static uint32_t table2[256];
static uint32_t table3[256];
//加密函数
void encryption(uint32_t* ciphertext) {
    uint32_t k[36];
    uint32_t x[4], tmp;
    for (int i = 0; i < 4; i++)
    {
    ...
    uint32_t tmp0, a;
    for (int i = 0; i < 32; i++) {
        rK[i] = k[i + 4] = k[i] ^ T_for_get_key_func(k[i + 3] ^ k[i + 2] ^ k[i + 1] ^ CK[i]);
        tmp0 = x[1] ^ x[2] ^ x[3] ^ rK[i];
        tmp = x[0];
        tmp ^= Table0[(tmp0 >> 24) & 0xFF];
        tmp ^= Table1[(tmp0 >> 16) & 0xFF];
        tmp ^= Table2[(tmp0 >> 8) & 0xFF];
        tmp ^= Table3[(tmp0 >> 0) & 0xFF];
...
void generate_table() {
    ...
    {
        uint8_t* A = (uint8_t*)&a;
        table0[i] = Cycle_Shift_lift(S_replace(A[0]), 24);
        table1[i] = Cycle_Shift_lift(S_replace(A[1]), 16);
        table2[i] = Cycle_Shift_lift(S_replace(A[2]), 8);
        table3[i] = Cycle_Shift_lift(S_replace(A[3]), 0);
        a++;
    }
}

void generate_table1() {
    uint32_t a = 0;
    for (int i = 0; i < 256; i++)
    {
        uint8_t* A = (uint8_t*)&a;
        ...
        B_8_t[3] = S_replace(A[3]);
        uint32_t* B_32_t = (uint32_t*)B_8_t;
        uint32_t b = Cycle_Shift_lift(*B_32_t, 24) ^ Cycle_Shift_lift(*B_32_t, 18) ^ Cycle_Shift_lift(*B_32_t, 10) ^ Cycle_Shift_lift(*B_32_t, 2);
        table0[i] = Cycle_Shift_lift(b, 24);
        table1[i] = Cycle_Shift_lift(b, 16);
        table2[i] = Cycle_Shift_lift(b, 8);
        table3[i] = b;
        //table3[i] = Cycle_Shift_lift(S_replace(A[0]), 24) ^ Cycle_Shift_lift(S_replace(A[1]), 18) ^ Cycle_Shift_lift(S_replace(A[2]), 10) ^ Cycle_Shift_lift(S_replace(A[0]), 2) << 0;
        a++;
    }
}
uint32_t L(uint32_t num)
{
    num = Cycle_Shift_lift(num, 2) ^ Cycle_Shift_lift(num, 10) ^ Cycle_Shift_lift(num, 18) ^ Cycle_Shift_lift(num, 24) ^ num;
    return num;
}

void generate_table2()
{
    for (int i = 0; i < 256; i++)
    {
        table0[i] = L(((uint32_t)S_replace(i)) << 24);
        table1[i] = L(((uint32_t)S_replace(i)) << 16);
        table2[i] = L(((uint32_t)S_replace(i)) << 8);
        table3[i] = L(((uint32_t)S_replace(i)));
    }
}
...
int main()
{
    ...
    for (int i = 0; i < 1000; i++)
        encryption(ciphertext_32);

    QueryPerformanceCounter(&EndingTime);
    ElapsedMicroseconds.QuadPart = EndingTime.QuadPart - StartingTime.QuadPart;

    double timeSpend = (ElapsedMicroseconds.QuadPart * 1000.0) / Frequency.QuadPart;
    cout << "encryption time:" << timeSpend << "ms" << endl;
    for (int i = 0; i < 4; i++) {
        uint8_t* ciphertext_8 = (uint8_t*)&ciphertext_32[i];
        dump_buf(ciphertext_8, 4);
    }
